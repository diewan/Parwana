...
pub fn generate_proof_leaf_vectors() -> Vec<ProofLeafVector> {
    vec![
        ProofLeafVector {
            ...
            expected_hash_ethereum: compute_hash_for_chain(
                &ProofLeafV1::new(
                    "bitcoin".to_string(),
                    "ethereum".to_string(),
                    Hash([0u8; 32]),
            ...
            expected_hash_solana: compute_hash_for_chain(
                &ProofLeafV1::new(
                    "bitcoin".to_string(),
                    "ethereum".to_string(),
                    Hash([0u8; 32]),
            ...
            expected_hash_sui: compute_hash_for_chain(
                &ProofLeafV1::new(
                    "bitcoin".to_string(),
                    "ethereum".to_string(),
                    Hash([0u8; 32]),
            ...
            expected_hash_aptos: compute_hash_for_chain(
                &ProofLeafV1::new(
                    "bitcoin".to_string(),
                    "ethereum".to_string(),
                    Hash([0u8; 32]),
            ...
            expected_hash_bitcoin: compute_hash_for_chain(
                &ProofLeafV1::new(
                    "bitcoin".to_string(),
                    "ethereum".to_string(),
                    Hash([0u8; 32]),
...
mod tests {
    ...
    fn test_proof_leaf_hash_deterministic() {
        ...
        for vector in &vectors {
            ...
            assert_eq!(
                hash_eth1, hash_eth2,
                "Ethereum hash should be deterministic for {}",
                vector.name
            );
            assert_eq!(
                hash_eth1, vector.expected_hash_ethereum,
                "Ethereum hash should match expected for {}",
                vector.name
            );
            ...
            assert_eq!(
                hash_sol1, hash_sol2,
                "Solana hash should be deterministic for {}",
                vector.name
            );
            assert_eq!(
                hash_sol1, vector.expected_hash_solana,
                "Solana hash should match expected for {}",
                vector.name
            );
            ...
            assert_eq!(
                hash_sui1, hash_sui2,
                "Sui hash should be deterministic for {}",
                vector.name
            );
            assert_eq!(
                hash_sui1, vector.expected_hash_sui,
                "Sui hash should match expected for {}",
                vector.name
            );
            ...
            assert_eq!(
                hash_aptos1, hash_aptos2,
                "Aptos hash should be deterministic for {}",
                vector.name
            );
            assert_eq!(
                hash_aptos1, vector.expected_hash_aptos,
                "Aptos hash should match expected for {}",
                vector.name
            );
            ...
            assert_eq!(
                hash_btc1, hash_btc2,
                "Bitcoin hash should be deterministic for {}",
                vector.name
            );
            assert_eq!(
                hash_btc1, vector.expected_hash_bitcoin,
                "Bitcoin hash should match expected for {}",
                vector.name
            );
        }
    }
            ...
            for (i, hash_i) in hashes.iter().enumerate() {
                for (j, hash_j) in hashes.iter().enumerate() {
                    if i != j {
                        assert_ne!(
                            hash_i, hash_j,
                            "Different inputs should produce different hashes for {:?}",
                            hash_fn
                        );
                    }
                }
    ...
    fn test_proof_leaf_cross_chain_consistency() {
        ...
        let chain_pairs: Vec<(&str, &str)> = vectors
            .iter()
            .map(|v| {
                (
                    v.leaf.source_chain.as_str(),
                    v.leaf.destination_chain.as_str(),
                )
            })
            .collect();
    ...
    fn test_proof_leaf_chain_specific_hashes_differ() {
        ...
        for vector in &vectors {
            ...
            assert!(
                unique_hashes.len() >= 4,
                "Chain-specific hashes should differ for {}",
                vector.name
            );
        }
    }
    ...
    fn test_proof_leaf_matches_production_implementation() {
        ...
        assert_eq!(
            hash_prod.0, hash_test,
            "Test hash should match production implementation"
        );
    }
}
