...
pub trait CanonicalEncoding {
    ...
    fn decode(bytes: &[u8], format: EncodingFormat) -> CodecResult<Self> where Self: Sized;
    ...
    fn decode_mce(bytes: &[u8]) -> CodecResult<Self> where Self: Sized {
    ...
    fn decode_manual(bytes: &[u8]) -> CodecResult<Self> where Self: Sized {
...
impl ManualEncoder {
    ...
    pub fn decode_bytes(bytes: &[u8], pos: &mut usize) -> CodecResult<Vec<u8>> {
        ...
        if bytes.len() < *pos + len {
            return Err(CodecError::DeserializationError("Insufficient bytes for data".to_string()));
        }
    ...
    pub fn decode_option_bytes(bytes: &[u8], pos: &mut usize) -> CodecResult<Option<Vec<u8>>> {
        if bytes.len() < *pos + 1 {
            return Err(CodecError::DeserializationError("Insufficient bytes for option flag".to_string()));
        }
    ...
    pub fn decode_hash(bytes: &[u8], pos: &mut usize) -> CodecResult<[u8; 32]> {
        if bytes.len() < *pos + 32 {
            return Err(CodecError::DeserializationError("Insufficient bytes for hash".to_string()));
        }
        let mut hash = [0u8; 32];
...
impl MCEEncoder {
    ...
    pub fn decode_fixed_bytes<const N: usize>(bytes: &[u8], pos: &mut usize) -> CodecResult<[u8; N]> {
        if bytes.len() < *pos + N {
            return Err(CodecError::DeserializationError(format!("Insufficient bytes for fixed-width array of {}", N)));
        }
        let mut data = [0u8; N];
