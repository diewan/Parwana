...
fn replay_is_impossible() {
    ...
    let consumption2 = SealConsumption {
        chain: chain.clone(),
        seal_ref: seal.clone(),
        sanad_id: SanadId(Hash::new([0xEF; 32])),
        block_height: 200,
    ...
    assert!(
        matches!(err.as_ref(), Box::<DoubleSpendError>(_) if true),
        "Error must be a DoubleSpendError"
    );
...
fn rollback_replay_is_detected() {
    ...
    let sanad_id = SanadId(Hash::new([0x11; 32]));
    let chain = ChainId::new("bitcoin");
    ...
    let c2 = SealConsumption {
        chain: chain.clone(),
        seal_ref: seal.clone(),
        sanad_id: SanadId(Hash::new([0x22; 32])),
