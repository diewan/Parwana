...
use csv_runtime::execution_journal::{ExecutionJournal, InMemoryJournal, PhaseOutcome, TransferPhaseEntry};
...
async fn test_journal_records_all_phase_transitions() {
    ...
    let replay_id = ReplayIdHash([1u8; 32]);

    // Record phase: Initialized (Entered)
    journal.record(TransferPhaseEntry {
    ...
    // Verify transfer is not incomplete (last phase was Completed)
...
async fn test_journal_identifies_incomplete_transfers() {
    ...
    let replay_id = ReplayIdHash([2u8; 32]);
...
async fn test_journal_records_failed_phase() {
    ...
    let replay_id = ReplayIdHash([3u8; 32]);
...
async fn test_journal_latest_phase_for_completed_transfer() {
    ...
    let replay_id = ReplayIdHash([4u8; 32]);
...
async fn test_journal_multiple_transfers() {
    ...
    let replay_id_1 = ReplayIdHash([1u8; 32]);
    journal.record(TransferPhaseEntry {
    ...
    let replay_id_2 = ReplayIdHash([2u8; 32]);
    journal.record(TransferPhaseEntry {
    ...
    let replay_id_3 = ReplayIdHash([3u8; 32]);
    journal.record(TransferPhaseEntry {
...
async fn test_journal_capacity_enforcement() {
    ...
    for i in 0..10 {
        journal.record(TransferPhaseEntry {
            ...
            replay_id: ReplayIdHash([i as u8; 32]),
            proof_hash: [0u8; 32],
            phase: TransferStage::Initialized,
            ts: std::time::SystemTime::now(),
            outcome: PhaseOutcome::Completed,
    ...
    let result = journal.record(TransferPhaseEntry {
        ...
        replay_id: ReplayIdHash([10u8; 32]),
        proof_hash: [0u8; 32],
        phase: TransferStage::Initialized,
        ts: std::time::SystemTime::now(),
        outcome: PhaseOutcome::Completed,
