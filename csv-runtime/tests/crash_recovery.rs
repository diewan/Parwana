...
use csv_runtime::adapter_registry::{
    AdapterError, AdapterRegistry, CrossChainTransfer, LockResult, MintResult, SealRegistryStatus,
};
use csv_runtime::event_bus::EventBus;
use csv_runtime::transfer_coordinator::TransferCoordinator;
...
impl AdapterRegistry for MockAdapterRegistry {
    fn capabilities(&self, _chain_id: &str) -> Option<csv_protocol::finality::ChainCapabilities> {
        Some(csv_protocol::finality::ChainCapabilities::ethereum())
    }
    ...
    async fn get_balance(&self, _chain_id: &str, _address: &str) -> Result<String, AdapterError> {
        Ok("1000".to_string())
    }
...
fn runtime_ctx(transfer: &CrossChainTransfer) -> csv_runtime::lease::RuntimeExecutionContext {
    let runtime_id = uuid::Uuid::new_v4();
    let now = std::time::SystemTime::now();
    csv_runtime::lease::RuntimeExecutionContext {
        lease: csv_runtime::lease::TransferLease::acquire(
        ...
        policy: csv_runtime::RuntimePolicy::default(),
    }
}
...
fn minimal_proof_bundle() -> Result<csv_protocol::proof_types::ProofBundle, String> {
    ...
    ProofBundle::new(
        DAGSegment::new(vec![node], Hash::new([0u8; 32])),
        vec![vec![0u8; 64]],
        SealPoint::new(vec![0u8; 32], Some(0))?,
        CommitAnchor::new(vec![0u8; 32], 100, vec![])?,
        InclusionProof::new(vec![], Hash::new([0u8; 32]), 100, 0)?,
        FinalityProof::new(vec![0u8; 32], 6, true)?,
    )
}
...
async fn crash_recovery_before_lock() {
    ...
    coordinator
        .execution_journal()
        .record(csv_runtime::execution_journal::TransferPhaseEntry {
...
async fn crash_recovery_after_replay_persist_before_mint() {
    let replay_db = InMemoryReplayDb::new();
    let replay_id_bytes = [1u8; 32];
    replay_db.insert_if_absent(&replay_id_bytes).await.unwrap();
    let replay_db = Box::new(replay_db);
    let event_bus = EventBus::new();
    let coordinator = TransferCoordinator::new(replay_db, event_bus);
    let adapter = Arc::new(MockAdapterRegistry);
