...
use crate::rpc::{
    AptosLedgerReader, AptosTransaction, AptosTransactionReader, AptosTransactionSubmitter,
};
...
impl AptosSealProtocol {
    ...
    pub fn with_rpc_and_signing_key(
        ...
        let rpc: Box<dyn AptosRpc> =
            Box::new(AptosNode::with_signer_address(rpc_url, sender_address));
    ...
    pub async fn build_and_sign_entry_function(
        ...
        let (sequence_number, ledger) = {
            ...
            loop {
                log::debug!(
                    "APTOS: Fetching account sequence number (attempt {}/{})",
                    retry_count + 1,
                    max_retries
                );
                match self.rpc.get_account_sequence_number(sender).await {
                    ...
                    Err(e) => {
                        ...
                        if retry_count >= max_retries {
                            return Err(format!(
                                "Failed to get sequence number after {} attempts: {}",
                                max_retries, e
                            )
                            .into());
                        }
    ...
    pub async fn sign_entry_function_payload(
        ...
        let json_args: Vec<serde_json::Value> = payload
            .arguments
            .iter()
    ...
    /// Sign, submit, and confirm an entry function. A stale sequence number
    /// invalidates the signature, so every retry refetches, rebuilds, and
    /// re-signs the transaction.
    #[cfg(feature = "rpc")]
    pub(crate) async fn submit_entry_function_with_retry(
        &self,
        payload: crate::entry_function::EntryFunctionPayload,
        ...
        for attempt in 1..=MAX_ATTEMPTS {
            let signed = self.sign_entry_function_payload(payload.clone()).await?;
            match self.rpc.submit_signed_transaction(signed).await {
                Ok(tx_hash) => {
                    let transaction =
                        self.rpc.wait_for_transaction(tx_hash).await.map_err(|e| {
                            format!(
                                "Failed to confirm transaction 0x{}: {e}",
                                hex::encode(tx_hash)
                            )
                        })?;
                    if !transaction.success {
                        return Err(format!(
                            "Aptos transaction 0x{} failed: {}",
                            hex::encode(tx_hash),
                ...
                Err(error) => {
                    ...
                    if attempt < MAX_ATTEMPTS && Self::is_sequence_number_too_old_error(&message) {
                        log::warn!(
                            "APTOS: {} rejected with stale sequence number; refetching and retrying ({}/{})",
                            payload.function_short_name(),
                            attempt,
                            MAX_ATTEMPTS
                        );
        ...
        Err(format!(
            "Failed to submit {} transaction after {MAX_ATTEMPTS} attempts",
            payload.function_short_name()
        )
        .into())
    }
...
impl SealProtocol for AptosSealProtocol {
    ...
    async fn publish(
        ...
        #[cfg(feature = "rpc")]
        {
            let builder = crate::entry_function::EntryFunctionBuilder::new(
                self.config.seal_contract.module_address.clone(),
            );
            let payload =
                builder.publish_sanad(seal.nonce, *sanad_id.as_bytes(), *commitment.as_bytes());
    ...
    async fn enforce_seal(
        ...
        #[cfg(all(feature = "rpc", not(test)))]
        {
            ...
            let seal_collection_data = self
                ...
                .ok_or_else(|| {
                    ProtocolError::SealReplay("Seal collection resource not found".to_string())
                })?;
    ...
    async fn create_seal(
        ...
        let nonce = if self.signing_key.is_some() {
            ...
            let seal_collection_type = format!(
                "0x{}::CSVSeal::SealCollection",
                self.config
                    .seal_contract
                    .module_address
                    .strip_prefix("0x")
                    .unwrap_or(&self.config.seal_contract.module_address)
            );
            log::debug!(
                "APTOS: Checking for seal collection resource type: {}",
                seal_collection_type
            );
            match self
                .rpc
                .get_resource(addr, &seal_collection_type, None)
                .await
            {
                Ok(None) => {
                ...
                Err(e) => {
                    log::warn!(
                        "APTOS: Failed to check seal collection: {}, assuming it exists",
                        e
                    );
                }
            }
            ...
            loop {
                match self.deploy_seal_on_chain(addr, effective_nonce).await {
                    Ok(()) => {
                        log::debug!(
                            "APTOS: Seal deployed on-chain successfully with nonce {}",
                            effective_nonce
                        );
                        break;
                    }
                    Err(e) => {
                        ...
                        if error_str.contains("EAnchorDataExists") && retry_count < max_retries {
                            log::warn!(
                                "APTOS: AnchorData already exists for nonce {}, incrementing and retrying (attempt {}/{})",
                                effective_nonce,
                                retry_count + 1,
                                max_retries
                            );
    ...
    fn hash_commitment(
        ...
        let core_seal = CoreSealPoint::new(
            seal_point.account_address.to_vec(),
            Some(seal_point.nonce),
            None,
        )
    ...
    async fn build_proof_bundle(
        ...
        if signatures.is_empty() {
            log::warn!(
                "APTOS: No signatures found in transition_dag - proof bundle may not be verifiable"
            );
        } else {
            log::info!(
                "APTOS: Extracted {} signatures from DAGSegment",
                signatures.len()
            );
        }
...
impl AptosSealProtocol {
    ...
    #[cfg(feature = "rpc")]
    pub async fn get_seal_from_chain(
        &self,
        account_address: [u8; 32],
    ) -> Result<AptosSealPoint, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if !response.status().is_success() {
            return Err(
                format!("Failed to query seal resource: HTTP {}", response.status()).into(),
            );
        }
                ...
                if res_type.contains("CSVSeal::SealCollection") {
                    ...
                    if let Some(data) = resource.get("data") {
                        ...
                        if let Some(next_nonce) = next_nonce {
                            log::debug!(
                                "APTOS: Found SealCollection with next_nonce {}",
                                next_nonce
                            );
    ...
    async fn init_seal_collection(
        &self,
        _account_address: [u8; 32],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let payload = crate::entry_function::EntryFunctionPayload {
            function: format!(
                "{}::CSVSeal::init_seal_collection",
                self.config.seal_contract.module_address
            ),
            type_arguments: vec![],
            arguments: vec![],
        };
        self.submit_entry_function_with_retry(payload).await?;
        Ok(())
    }
    ...
    async fn deploy_seal_on_chain(
        ...
        #[cfg(feature = "rpc")]
        {
            ...
            match client
                .wait_for_transaction(&hash_value, Some(Duration::from_secs(20)))
                .await
            {
                Ok(response) => {
                    ...
                    log::debug!(
                        "APTOS: Transaction response received - success: {}, vm_status: {}",
                        success,
                        vm_status
                    );
                    if success {
                ...
                Err(e) => Err(format!("Failed to wait for transaction confirmation: {}", e).into()),
            }
        }
...
mod tests {
    ...
    #[tokio::test]
    #[cfg(feature = "rpc")]
    async fn entry_submission_refetches_and_resigns_after_stale_sequence() {
        use ed25519_dalek::SigningKey;

        let rpc = crate::rpc::MockAptosRpc::new(5000);
        rpc.reject_next_submissions_as_stale(1);
        let adapter = AptosSealProtocol::from_config(AptosConfig::default(), Box::new(rpc))
            .unwrap()
            .with_signing_key(SigningKey::from_bytes(&[1u8; 32]));
        let payload = crate::entry_function::EntryFunctionBuilder::new(
            adapter.config.seal_contract.module_address.clone(),
        )
        .publish_sanad(7, [0x11; 32], [0x22; 32]);
