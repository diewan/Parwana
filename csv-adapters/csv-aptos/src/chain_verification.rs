...
impl super::AptosBackend {
    /// Chain-native inclusion verification (accumulator/Merkle proof).
    ...
    pub fn verify_finality_native(
        ...
        #[cfg(feature = "rpc")]
        {
            ...
            // Aptos has deterministic finality via HotStuff (2f+1 validator signatures)
            // Check minimum confirmations (Aptos blocks are final once certified)
            if proof.confirmations < 1 {
                return Ok(false);
            }

            // Verify finality data contains ledger info
            ...
            // Validate tx_hash format (Aptos transaction hash)
...
impl ChainNativeProofVerifier for super::AptosBackend {
