//! Compile-fail test: Completed to ProofBuilding (backward transition)
//!
//! This test ensures that going backward from Completed to ProofBuilding
//! is impossible at compile time with csv-algebra typestate.

use csv_algebra::state::{Locked, Completed};
...
fn main() {
    ...
    let completed = minting.confirm();

    // This should fail to compile - cannot go backward from Completed to ProofBuilding
    // Completed has no method to go to ProofBuilding
    let _proof_building = completed.begin_proof(); // ERROR: no such method
}
