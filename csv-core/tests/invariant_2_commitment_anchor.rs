...
mod tests {
    ...
    fn test_commit_anchor_metadata_size_limit() {
        ...
        assert!(anchor.metadata.len() <= 4096, "metadata must not exceed MAX_ANCHOR_METADATA_SIZE");
    }

    /// Property: CommitAnchor with oversized metadata is rejected
    #[test]
    fn test_commit_anchor_oversized_metadata_rejected() {
        let result = CommitAnchor::new(
            vec![0xABu8; 32],
            1000,
            vec![0u8; 4097],
        );
    ...
    fn test_commit_anchor_serialization() {
        ...
        assert_eq!(bytes.len(), 8 + 32 + 64, "Serialized size must match expected");
    }
}
