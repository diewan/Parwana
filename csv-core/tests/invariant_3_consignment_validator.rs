...
mod tests {
    ...
    use csv_core::Hash;
    use csv_core::consignment::{CONSIGNMENT_VERSION, Consignment};
    ...
    fn minimal_consignment() -> Consignment {
        ...
        let genesis = Genesis::new(Hash::new([1u8; 32]), schema, vec![], vec![], vec![]);
    ...
    fn consignment_validator_runs_before_acceptance() {
        let c = minimal_consignment();
        let report = ConsignmentValidator::new().validate_consignment(&c, ChainId::new("bitcoin"));
