...
impl AdapterFactory for AptosFactory {
    async fn create_adapter(&self, config: AdapterConfig) -> Result<AdapterResult, FactoryError> {
        ...
        // Resolution is destination-chain-scoped (CSV_MINT_VERIFIER_KEY_APTOS
        // overrides the CSV_MINT_VERIFIER_KEY default for Aptos only) and may
        // carry multiple signers for an M-of-N registry. With none configured the
        ...
        if verifier_keys.is_empty() {
            ...
            log::info!(
                "Factory: Aptos adapter configured with {} mint verifier signer(s)",
                verifier_keys.len()
            );
            aptos_backend_inner = aptos_backend_inner.with_verifier_keys(verifier_keys);
        }
